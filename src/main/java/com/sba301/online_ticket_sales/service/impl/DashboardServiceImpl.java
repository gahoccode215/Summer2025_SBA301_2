...
public class DashboardServiceImpl implements DashboardService {
    ...
    @Override
    public DashboardResponse getManagerDashboard(Long cinemaId, LocalDate startDate, LocalDate endDate) {
    ...
    private DashboardOverview getAdminOverview(LocalDateTime startDate, LocalDateTime endDate) {
        ...
        return DashboardOverview.builder()
                ...
                .totalBookings(ticketOrderRepository.countByPaymentStatusAndCreatedAtBetween(
                        PaymentStatus.SUCCESS, startDate, endDate))
                .totalRevenue(ticketOrderRepository.sumRevenueByDateRange(
                        PaymentStatus.SUCCESS, startDate, endDate))
                .todayRevenue(ticketOrderRepository.sumRevenueByDateRange(
                        PaymentStatus.SUCCESS, todayStart, todayEnd))
                .todayBookings(ticketOrderRepository.countByPaymentStatusAndCreatedAtBetween(
                        PaymentStatus.SUCCESS, todayStart, todayEnd))
                .averageRating(null) // Bỏ rating
                .build();
    }

    private DashboardOverview getManagerOverview(Long cinemaId, LocalDateTime startDate, LocalDateTime endDate) {
        LocalDateTime todayStart = LocalDate.now().atStartOfDay();
        LocalDateTime todayEnd = LocalDate.now().atTime(LocalTime.MAX);
        ...
        return DashboardOverview.builder()
                ...
                .totalBookings(ticketOrderRepository.countByCinemaAndPaymentStatusAndDateRange(
                        cinemaId, PaymentStatus.SUCCESS, startDate, endDate))
                .totalRevenue(ticketOrderRepository.sumRevenueByCinemaAndDateRange(
                        cinemaId, PaymentStatus.SUCCESS, startDate, endDate))
                .todayRevenue(ticketOrderRepository.sumRevenueByCinemaAndDateRange(
                        cinemaId, PaymentStatus.SUCCESS, todayStart, todayEnd))
                .todayBookings(ticketOrderRepository.countByCinemaAndPaymentStatusAndDateRange(
                        cinemaId, PaymentStatus.SUCCESS, todayStart, todayEnd))
                .averageRating(null) // Bỏ rating
                .build();
    }

    private List<TopMovie> getTopMovies(Long cinemaId, LocalDateTime startDate, LocalDateTime endDate, int limit) {
        List<Object[]> results;

        if (cinemaId != null) {
            results = ticketOrderRepository.findTopMoviesByCinemaAndDateRange(
                    cinemaId, PaymentStatus.SUCCESS, startDate, endDate, PageRequest.of(0, limit));
        } else {
            results = ticketOrderRepository.findTopMoviesByDateRange(
                    PaymentStatus.SUCCESS, startDate, endDate, PageRequest.of(0, limit));
        }

        return results.stream()
                .map(result -> TopMovie.builder()
    ...
    private List<RevenueByDate> getRevenueChart(Long cinemaId, LocalDate startDate, LocalDate endDate) {
        List<Object[]> results;

        if (cinemaId != null) {
            results = ticketOrderRepository.getRevenueByCinemaAndDateRange(
                    cinemaId, PaymentStatus.SUCCESS, startDate, endDate);
        } else {
            results = ticketOrderRepository.getRevenueByDateRange(
                    PaymentStatus.SUCCESS, startDate, endDate);
        }

        return results.stream()
                .map(result -> RevenueByDate.builder()
    ...
    private List<CinemaPerformance> getCinemaPerformances(LocalDateTime startDate, LocalDateTime endDate) {
        List<Object[]> results = ticketOrderRepository.getCinemaPerformanceByDateRange(
                PaymentStatus.SUCCESS, startDate, endDate);

        return results.stream()
                .map(result -> CinemaPerformance.builder()
    ...
    private List<RecentBooking> getRecentBookings(Long cinemaId, int limit) {
        ...
        if (cinemaId != null) {
            results = ticketOrderRepository.findRecentBookingsByCinema(
                    cinemaId, PageRequest.of(0, limit));
        } else {
            results = ticketOrderRepository.findRecentBookings(PageRequest.of(0, limit));
        }

        return results.stream()
                .map(result -> RecentBooking.builder()
    ...
    private void validateCinemaAccess(Long cinemaId) {
        ...
        boolean hasAccess = currentUser.getManagedCinemas().stream()
